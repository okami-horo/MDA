...
## Current Status
Stable, actively maintained. Remote membership verification disabled on `develop` branch; all users now run with unlimited runtime. **Preserving the membership bypass is the top priority for every future change, including upstream merges.** Memory bank initialized to improve cross-session agent context. Latest `upstream/HEAD` (up to `19e227b`) has been merged into `develop` while preserving fork-specific bypass (`checkMembership` local unlimited), local `QuickBattleAvailable.count=80`, Memory Bank, and CI guards. The merge removed the quota carried-debt mechanism, fixed the timer-goroutine `PostStop` crash, and added SmallEvent PROJECT MATIS Story 2 Hard-stage templates.
